...
/*
Looks for a free frame (4 consecutive cell set to NULL) starting from the first
Frame in RAM, and checking the next frame and so on. If no such free frame
exist, return -1 Else a free frame was found, return its framenumber
*/
int findFrame();
...
victimFrame : If equal to -1, nothing
                          If not equal to -1, then it is the same as the
frameNumber but denotes that this frame belongs to another PCB being the victim
